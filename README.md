...
Moira（Μοῖραι — 希腊命运三女神之名）是一款开源的**中国传统占星学桌面应用**，基于 JPL DE440 高精度星历数据，提供完整的七政四余星盘计算、四柱八字排盘、神煞推演与星力分析。面向中国传统命理学研究者、天文爱好者与命理软件开发者的深度探索工具。
...
避免使用西方占星术语（星座名请用中国传统十二宫名称）。

---

## 项目状态

该项目处于 **早期开发阶段**。当前版本已实现完整的七政四余星盘计算、八字排盘、神煞系统等核心功能，正在进行界面优化和功能扩展。
...
版权所有 (C) 2025 Milk0217

本程序是自由软件，你可以遵照自由软件基金会（Free Software Foundation）出版的 **GNU 通用公共许可证第三版 (GNU General Public License v3)** 来修改和重新发布这一程序。

本程序分发时希望它会有用，但**不提供任何担保**；甚至没有对**适销性**或**特定用途的适用性**的默示担保。详见 GNU General Public License。

*本项目源自 Java 版 Moira（GPL v2 or later），根据原许可证条款可选择后续版本进行授权。*
